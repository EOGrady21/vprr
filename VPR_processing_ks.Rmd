...
# General comments - KS 02-03-2020

- Try to shorten the text wherever possible. I have tried to do this throughout. It may seem kind of brutal at first, but it is important to be concise. Especially when the process is complicated, like it is here.

- Avoid the use of "you" in the text. If you have to, use "the user"?

- There is a part where you talk about previous bugs in the code. Should that part have its own section?

-Directory structure (throughout): I need to think about this more, but I'm not sure if the R component of processing is as dependent on directory structures from VP as described? Its all in how you set your paths at the beginning, correct? Are there any functions where directories are hard coded in?

(1) Introduction

"The instrument" section can be shortened and merged into the introduction. The description of the VPR can be very brief, with reference to the manual published by Seascan. It is best to leave out information on the specs and just reference the manual. Please let me know if you do not have a copy of the manual (most recent and earlier versions).

Important points about "the instrument"

- Definition: VPR = Video Plankton Recorder
- The VPR is a Seascan product (provide website, contact info?, or link to manual?)
- There are several versions of the VPR, this code was developed for the DAVPR version
- VPR = CPU + camera + CTD (SBE49)
- Deployment can be in towyo or profile mode

Important general introduction points:

- A more detailed description of the raw data from the VPR
- A more detailed description of VP and its output (give website where VP software can be obtained, I will provide this). This is important at this point, because our code relies on VP structures. 
- This code was built for VP outputs, but can be generalized (can it?)
- An explanation that this package takes data outputs from the VPR (CTD data) and VP (roi ids in text files) and produces estimates of zooplankton concentration and CTD metrics, averaged in vertical bins along the VPR path. 

(2) Summary of processing steps

It needs to be made clear what types of data come into the vprr processing from the VPR and VP and what types of data/metadata come out of the vprr processing. Sometimes your refer to file types and file names later in the document. It needs to be made clear what your are talking about.

**I think that we should also include the "easyplot" functions in this package. What are your thoughts?

Also, it would be really helpful to extend vprr to accomodate those who do not want to deal with VP or other automated classification schemes, but want to go straight to manual classification. Is there a way that they could just import their data straight into clfcheck? We should talk about this.

(3) AutoID copying

Just a nomenclature thing, but I think our functions should be called "autoid"" rather than "aid".

(4) Krill check

I think we should omit this section. I would also like to think of solns to "go back" during classification to undo human errors. I think its possible.

#EC text with KS EDITS

# Introduction

This document was produced at Bedford Institute of Oceanography (BIO) to accompany the vprr package, a data processing and visualization package for the Video Plankton Recorder (VPR). It will summarize the data processing procedure followed at BIO and the data products produced. The scope of this document is focused on the portion of data processing done in R. Before data is cleaned and visualized in R it goes through initial image filtering software (autoDeck) and an optional machine learning classification software in MATLAB (Visual Plankton).

# The Instrument

The Visual Plankton Recorder (VPR) was initially designed at Woods Hole Oceanographic Institute (WHOI). It is an oceanographic instrument which combines the properties of CTD measurements and high quality underwater photography. The VPR itself contains two distinct instruments which log data simultaneously, the CTD and the camera. These two instruments work together to give a detailed picture of the microscopic plankton community in the ocean. 

The CTD which is connected to the VPR is a seaBird product (SBE49) and can accommodate many of the sensors typically found on a CTD which would be deployed with a Rosette. At BIO we have included fluorescence and turbidity sensors to accompany standard depth, temperature and salinity measurements. This CTD is consistently logging data throughout the VPR cast. 
...
This step is optional although it is recommended by folks at BIO have processed this data. The goal is to improve upon the automatic classifications manually correcting misclassified ROIs in taxa categories that have not been sorted adequately by VP. In this step, ROIs are displayed on the screen one at a time for manual classification. 

There is also an option to create new categories for classification, which is useful for rare taxa groupings. This is done by adding new classification groups to your 'taxa of interest' (`taxa_of_interest` variable in example below) then calling the function `vprr::add_new_taxa`. 

There are three steps of reclassification. 

## Step A - Manual classification
...
## Step B - Create new autoid files
...
## Step C - Run file check (this section is too long)
The last step of reclassification includes some manual file organization and final checks. Once all classification (aid & aidmeas) files have been created (with `vprr::new_aids`), they are contained in taxa folders inside your working directory. These files need to be reorganized within your original Visual Plankton directory (Do they?? You can put them anywhere at this point, like a new R project? As long as your have your paths set accordingly). 

You should use the new aid and aidmeas files created to replace the original VP files where you have performed reclassification. Remember this may not be for all classification groups, or all days/hours of data. You may also want to archive a copy of the original classification files, in case you need to rerun processing, this will prevent you from having to rerun Visual Plankton in MATLAB which can be time consuming.

KS:

I'm not sure this is best practise. What I have been doing is creating a new autoid folder in the VP directory and using that in the path arguments. But this folder could be anywhere. It does not need to be in the VP directory, correct? 
...
for (j in 1:length(all_stations_of_interest)){
 ...
 Optical settings and image volume variables should be set, if they are consistent throughout the cruise, they could also be added to the processing environment.
  ...
  taxa_conc_n <- calculate_vpr_concentrations(data, taxas_list, station_of_interest )
  
  cat('Station', station_of_interest, 'processing complete! \n')
  cat('\n')
  ```
  Finally, data is saved as RData and csv files for export and plotting. 
...
The last bit of set up required, sets the directory in which plots will be saved and some generic plot size arguments which will control how large the saved .png files are. 
  ...
  This first plot follows a consistent format for creating VPR contour plots with hydrographic contours behind VPR concentration data over the tow path. The main function used is `vprr::conPlot_EC` which uses a standard VPR data frame (produced from Processing) and plots the background contours. Interpolation methods can be adjusted based on data or preference. Then the VPR tow path can be added on top of contours, this displays the concentration data as bubbles. This method can be repeated with various hydrographic parameters used to calculate the contours, by changing the `var` argument in `vprr::conPlot_EC`.  

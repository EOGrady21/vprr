...
Visual Plankton code can be found: On GC code (for internal DFO users), within the dfo-mar-odis group, and 'visual plankton' project. For permission to view the project on GC code, contact DataServicesDonnees@dfo-mpo.gc.ca.
...
Once images have been properly classified, all data sources are combined in order to analyze data and calculate relevant environmental metrics such as plankton concentration. Data files from CTD files (`vpr_ctd_read`) and image classification (`vpr_autoid_read`) can be read in and combined into easy to work with data frames. VPRR combines VPR CTD, and VPR image classifications into bins before calculating concentration (`vpr_roi_concentration`). 

...
data obtained from the Digital Auto Video Plankton Recorder (VPR) produced by SeaScan
...
VPR. The package does not include automated image classification; however, 
there is an optional manual classification module, which can be used to review 
and correct outputs from automated image classification while providing a record 
of any (re)classifications.
...
categories manually or by an automated classification procedure. In vprr, 
file naming conventions and directory structures are inherited from a VPR 
image classification and analysis software, Visual Plankton. However, 
the functionality of vprr is not dependent on the use of VP. 


The data inputs for processing in vprr consist of the following file types:
aid (.txt), aidmeas (.txt), and CTD (.dat). The aid and aidmeas files are 
derived from separate image classification and measurement steps outside 
of vprr.  Each "aid" (i.e., autoid) file contains
...
data for the ROIs (e.g., long axis length, perimeter, etc.). 
...
The processing steps in R from the 'Post Processing
and Visualization box in Figure 1 are detailed in Figure 2. 

![Figure 2. Processing steps in R using vprr. Blue boxes represent each section of processing, 
green ovals represent sub steps or routines within each step.](r_flowchart.png)
...
links station names from deck sheets to the day and hour values assigned by
AutoDeck. Day and hour values represent the Julian day (3 digit)
...
information in each aid file is used to create a folder of images that have been classified to
...
classification in VP. ROIs that have been copied in Section 2.1 are 
manually sorted to correct for misclassifications made
by automated classification. Updated aid and aidmeas files are produced. 
Further details on manual re-classification are provided in Section 4.

## Section 2.4: Processing:

Joining data outputs from Autodeck (ctd .dat files) and automatic classifications (aid files)
and measurements (aidmeas files). The aid and aidmeas files, which may have been updated
...
from automatic classification (see Appendix 1: 'Image Folders'). The images are organized by
...
inspection of classifications. Moreover, this directory structure
...
for manual verification. If an image has been misclassified or if it falls
...
were not defined prior to automatic classification. After completing manual re-classification 
for a day-hour set, new aid and aidmeas files are created for new categories, which are
identical in format to original aid and aidmeas files.

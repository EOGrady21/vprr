...
Krill check requires 3 parts. First, images must be pulled from reclassification into new folders which can be visually checked. Second, images hsould be manually sorted in file explorer. This requires deleting any non - krill images from the folders created in the first part of krill check. Thirdly, the manually organized folders must be compared with aid and aidmeas files and any ROI strings which are no longer present in folders should be removed from text files (aid and aidmeas). 
...
for (j in 1:length(all_stations_of_interest)){
 ...
 Optical settings and image volume variables should be set, if they are consistent throughout the cruise, they could also be added to the processing environment.
  ...
  ROI and measurmeent data files are then read using `read_aid`.
  ...
  ```
  Before final export of data products, a few calculated varibales are added, `avg_hr` is a variable representing time in hours rather than milliseconds, which simplifies plotting and is more easily understood. `sigmaT` is also caluctaed using`oce::swSigmaT` to give a more complete picture of hydrographic properties. 

...
There are a series of functions written to help achieve this goal. VPR images are displayed on the screen one at a time for classification, if VP has misclassified an image or if it can be recognized as falling into a finer resolution category the image can be reclassified. At the end of this process text files, which match the formats produced by VP are reproduced with new classifications. 

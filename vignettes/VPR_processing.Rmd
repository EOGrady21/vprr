...
Data outputs from Autodeck (ctd .dat files), automated classifications (aid files)
...
```
        ...
        # Once classified images are sorted by category
        ...
        vpr_category_create(category = category_of_interest, auto_id_folder)
        ...
        vpr_manual_classification(day = day, hour= hr, basepath = auto_id_folder,gr = FALSE, 
                  category_of_interest = category_of_interest, scale = 'x300',
                  opticalSetting = 'S3')
...
and metadata in an efficient format.
      ...
      write.csv(file = paste0(stdir, '/vpr_data_binned', station_of_interest, '.csv'), category_conc_n) # VPR and CTD data with concentrations by category
...
generated using a loop. This example isolates two specific
classification categories to plot (eg. "Calanus" and "krill").

    category_to_plot <- c("Calanus", "krill")
    station <- "example"
...
csv file.
     ...
     # load concentration data
      category_conc_n <- read.csv(fn_conc, stringsAsFactors = F)
...
(`category_conc_n` - produced from processing (Section 5)) and plots the
...
the `var` argument in `vprr::vpr_plot_contour()`.
      ...
      # Density (sigmaT)
      png('conPlot_category_dens.png', width = width, height = height)
      p <- vpr_plot_contour(category_conc_n[category_conc_n$category %in% c(category_to_plot),], var = 'density', dup = 'strip', method = 'oce', bw = 0.5)
      p <- p + geom_line(data = data, aes(x = time_hr - min(time_hr), y = pressure), col = 'snow4', inherit.aes = FALSE) +
        geom_point(data = category_conc_n[category_conc_n$category %in% c(category_to_plot),], aes(x = time_hr, y = min_pressure, size = conc_m3), alpha = 0.5)+
...
pattern in vertical distribution over the VPR deployment.
        ...
        p <- vpr_plot_profile(category_conc_n, category_to_plot)
        print(p)
        dev.off()
...
ggplot2 package for more information).
    ...
    ####TS BUBBLE PLOT ----------------------------------------------------------------------------------------------------------------------------
      ...
      # plot by category
      category_conc <- category_conc_n[category_conc_n$conc_m3 > 0,]
      png('TS_conc_category.png', width = 1000, height = 500)
      p <- vpr_plot_TS(category_conc[category_conc$category %in% c(category_to_plot),], var = 'conc_m3') +
...
**Auto ID** - The automated classification given to an image from Visual
Plankton machine learning algorithm

**AutoID files** - Includes both Aid and AidMeas files as part of Visual
Plankton's automated classifications
